...
class Conv2d(nn.Module):
    def __init__(self,in_c,out_c,k_size,stride=1,activate=True,padding=1,bn=False,In=False):
        ...
        self.In = nn.InstanceNorm2d(out_c, track_running_stats=True, affine=True) if In else None
        self.relu = nn.ReLU() if activate else None
...
class SAModule(nn.Module):
    ...
    def __init__(self,in_c,out_c,In=True,middleLayer=True):
        ...
        self.branch_1 = Conv2d(in_c,out_c,1,padding=0,In=True)
        if not middleLayer:
            self.branch_2 = Conv2d(in_c,out_c,3,padding=1,In=True)
            self.branch_3 = Conv2d(in_c,out_c,5,padding=2,In=True)
            self.branch_4 = Conv2d(in_c,out_c,7,padding=3,In=True)
        else:
            self.branch_2 = nn.Sequential(
                Conv2d(in_c,out_c//2,1,padding=0,In=True),
                Conv2d(out_c//2,out_c,3,padding=1,In=True)
            )
            self.branch_3 = nn.Sequential(
                Conv2d(in_c,out_c//2,1,padding=0,In=True),
                Conv2d(out_c//2,out_c,5,padding=2,In=True)
            )
            self.branch_4 = nn.Sequential(
                Conv2d(in_c,out_c//2,1,padding=0,In=True),
                Conv2d(out_c//2,out_c,7,padding=3,In=True)
            )
